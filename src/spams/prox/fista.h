...
namespace FISTA {
   ...
   enum regul_t { L0, L1, RIDGE, TIKH, L2, LINF, L1CONSTRAINT, ELASTICNET, FUSEDLASSO, GROUPLASSO_L2, GROUPLASSO_LINF, GROUPLASSO_L2_L1, GROUPLASSO_LINF_L1, L1L2, L1LINF, L1L2_L1, L1LINF_L1, TREE_L0, TREE_L2, TREE_L2_RIDGE,TREE_L2_TIKH, TREE_LINF, GRAPH, GRAPH_RIDGE, GRAPH_L2, TREEMULT, GRAPHMULT, L1LINFCR, NONE, TRACE_NORM, TRACE_NORM_VEC, RANK, RANK_VEC, INCORRECT_REG, GRAPH_PATH_L0, GRAPH_PATH_CONV, LOG_DC, NA};
   ...
   regul_t regul_from_string(char* regul) {
	  ...
	  if (strcmp(regul,"tikh")==0) return TIKH;
	  ...
	  if (strcmp(regul,"tree-l2-tikh")==0) return TREE_L2_TIKH;
      if (strcmp(regul,"tree-l2-ridge")==0) return TREE_L2_RIDGE;
   ...
   void print_regul(const regul_t& regul) {
      switch (regul) {
         ...
         case TIKH: cout << "Tikhonov regularization" << endl; break;		 
		 ...
		 case TREE_L2_TIKH: cout << "Tree-L2 + Tikhonov regularization" << endl; break;
         case TREE_L2_RIDGE: cout << "Tree-L2+ridge regularization" << endl; break; 
   ...
   template <typename T> struct ParamFISTA { 
      ...
      int ngroups;
      int linesearch_mode;
	  AbstractMatrixB<T> *choler;
	  Matrix<T>  *invcholer;
	  Matrix<T> *eigenvalinv;
   };
...
//            virtual bool is_none() const { return true; };
   ...
   template <typename T> 
	  ...
	  template <typename T> 
      class Tikh: public Regularizer<T> {
         public:
            Tikh(const ParamReg<T>& param) : Regularizer<T>(param) { this->_id = TIKH; };
            virtual ~Tikh() { };
			...
			/*DK Edit only need this to work within treelasso, so no fenchel or subgrad needed*/
			            virtual bool is_fenchel() const { return false; };
			void inline fenchel(const Vector<T>& input, T& val, T& scal) const {
               Vector<T> tmp;
               tmp.copy(input);
   ...
   template <typename T>
      struct TreeL2Ridge {
         typedef ComposeProx<T, Vector<T>, TreeLasso<T>, Ridge<T>, true> type;
      };
...
#pragma omp critical
   ...
   template <typename T, typename D, typename E>
      void FISTA_Generic(Loss<T,D,E>& loss, Regularizer<T,D>& regularizer, const D& x0, D& x, Vector<T>& optim_info,
		...
		/*storage for matrix that will be multiplied for the proxmial operator of a Tikhonov regularization*/
		Matrix<T> AAA_t, AAA, BBB, CCC, DDD;
         const bool duality = loss.is_fenchel() && regularizer.is_fenchel();
         T rel_duality_gap=-INFINITY;
         optim_info.set(-1);
         ...
         for (it = 1; it<=param.max_it; ++it) {
            ...
            if (param.verbose && ((it % it0) == 0)) {
			   ...
			   {
				   //D extra;
				   //param.choler->mult(x,extra);
				   los=loss.eval(x);/* + lambda*regularizer.eval(extra);*/
			   }
			   else if(param.regul==TREE_L2_TIKH)
			   {
				   //D extra;
				   //param.choler->mult(x,extra);
				   los=loss.eval(x)+ lambda*regularizer.eval(x);/* + lambda*regularizer.eval(extra);*/
			   }
			   else
			   {
					los=loss.eval(x) + lambda*regularizer.eval(x); 
			   }
			   cout << los<< endl;
               optim_info[0]=los;
               T sec=time.getElapsed();
            ...
            while (iter < param.max_iter_backtracking) {
			    ...
			    if(param.regul==TIKH)
			   {
				   ...
				   Vector<T> diagstore;
					/*store eigenvectors*/
				   param.invcholer->transpose(AAA_t);
				   param.invcholer->copyTo(AAA);
				   param.eigenvalinv->copyTo(BBB);
				   BBB.scal(T(1.0)/L);
				   BBB.incrDiag();
				   BBB.diag(diagstore);
				   ...
				   diagstore.inv();
				   BBB.setDiag(diagstore);
				   AAA.mult(BBB,CCC);
				   CCC.mult(AAA_t,DDD);
	//			   DDD.diag(diagstore);
				   /*for debugging, char* if vector string if matrix*/
	//	   			char * printer;
	//			   	diagstore.print(printer);
	//			   	cout<<printer<<endl;
				   DDD.mult(prox,tmp);
					//regularizer.prox(prox,tmp,lambda/L);
	...
	//			   	cout<<printer<<endl;
				   DDD.mult(prox,tmp1);
				   regularizer.prox(tmp1,tmp,lambda/L);
					/*I MIGHT NEED TO INSTEAD USE lambda/(T(1.0)+lambda) instead of lambda there*/
			   }
			   else
			   {
					regularizer.prox(prox,tmp,lambda/L);
   ...
   template <typename T>
      Regularizer<T>* setRegularizerVectors(const ParamFISTA<T>& param,
         ...
         switch (param.regul) {
			...
			case TIKH: reg=new Tikh<T>(param_reg); break;
			...
			case TREE_L2_TIKH: param_reg.linf=false; reg=new TreeLasso<T>(param_reg); break;
			case TREE_L2_RIDGE: param_reg.linf=false; reg=new typename TreeL2Ridge<T>::type(param_reg); break;
   ...
   template <typename T>
      Regularizer<T, Matrix<T> >* setRegularizerMatrices(const ParamFISTA<T>& param,
         ...
         switch (param.regul) {
			...
			case TREE_L2_TIKH: param_reg.linf=false; reg=new RegMat<T, TreeLasso<T> >(param_reg); break;
            case TREE_L2_RIDGE: param_reg.linf=false; reg=new RegMat<T, typename TreeL2Ridge<T>::type >(param_reg); break;
